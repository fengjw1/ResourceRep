!(function () {
	...
	var api = {
	  ...
	  createQRCodeDirect: function(content){
        var that = this;
		// var pixels = new ArrayBuffer(width*width);
		
        var str = that.utf16to8(content);
		var frame = that.getFrame(str);
		var pixelsView = new Uint8Array(width*width);
		console.log("width>>" + width);
        for (var i = 0; i < width; i++) {
          for (var j = 0; j < width; j++) {
              if(frame[j * width + i]){
                  pixelsView[j * width + i] = 1;
              }else{
                pixelsView[j * width + i] = 0;
              }
          }
        }
        return pixelsView;
